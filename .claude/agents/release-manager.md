...
* 작업 트리에 커밋되지 않은 변경이 있으면 **목록을 보여주고 무엇을 포함할지
  묻는다.** 단 `sealock/version.py` 는 예외다 — 시험용으로 낮춰둔 값이든
  무엇이든, 7단계에서 `X.Y.Z+dev` 로 덮어쓴다고 미리 알려주기만 한다.
...
### 7. 버전 자리표시자 정리 (발행이 확인된 뒤)

리포에 커밋돼 있는 `sealock/version.py` 의 값은 릴리즈 결과물과 무관하다 — CI 가
태그에서 뽑아 덮어쓰기 때문이다. 그래서 아무도 눈치채지 못한 채 낡는다. 실제로
1.0.0 을 내보낸 뒤에도 `0.0.3+dev` 로 몇 버전을 방치했고, 시험하느라 `0.0.2` 로
낮춘 값이 그대로 남아 있기도 했다. 릴리즈를 마무리하며 여기서 맞춘다.

방금 낸 버전에 `+dev` 를 붙인 값으로 쓴다 — `X.Y.Z+dev`:

* `+dev` 가 "릴리즈 빌드 아님" 표시라 개발 빌드에서 자동 업데이트가 잠긴다
  (`updater.is_dev_build` / `can_self_update`).
* 숫자 부분은 방금 발행한 버전과 같아야 한다. 다음 버전(`X.Y.Z+1+dev`)을 미리
  적으면 그 버전이 실제로 나왔을 때 `is_newer` 가 거짓이 되어, 개발 빌드가
  자기보다 새 릴리즈를 못 본다.

파일은 **자리표시자 문자열만** 바꾼다. docstring(자리표시자 규칙과 SEALOCK_VERSION
사용법을 설명하는 유일한 글)과 환경변수 override 구조는 그대로 둔다:

```python
__version__ = os.environ.get("SEALOCK_VERSION", "").strip() or "X.Y.Z+dev"
```

```bash
git add sealock/version.py
.venv/bin/python scripts/pii_check.py
git commit -m "chore: 버전 자리표시자를 X.Y.Z+dev 로"
git push origin master
```

이 커밋은 태그 **뒤**에 온다. 태그는 릴리즈 노트를 확정한 커밋을 가리키고,
자리표시자 정리는 릴리즈 후 뒷정리로 남는 편이 이력을 읽기 쉽다.

## 보고

끝나면 이렇게 정리한다: 태그, 릴리즈 URL, 세 잡의 결과, 첨부 파일 두 개와 크기,
CA 번들 확인 결과, CHANGELOG 에 확정한 날짜, `version.py` 를 무엇에서 무엇으로
바꿨는지. 도중에 멈췄다면 **어디서 왜 멈췄는지**와 사용자가 할 수 있는 다음
선택지를 적는다.
